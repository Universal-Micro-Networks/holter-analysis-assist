...
- [x] 2. Core: 設定・エラー・ヘルスの独立部品
- [x] 2.1 (P) HTTP 用 ini 設定を読み取り既定上限を固定する
...
- [x] 2.2 (P) 失敗区分と CSV／JSON 応答変換を用意する
...
- [x] 5. Validation: 契約と fail-closed の自動検証
- [x] 5.1 設定既定値・エラー区分・ヘルス非計上の単体テストを追加する
...
- [x] 5.2 起動非リッスン・解析成功／拒否・計上回数・過大ボディを結合検証する
...
- [x]* 5.3 実サーバー／埋め込みビルドでの手動スモーク手順を残す
