...
上流 `api-console-ui` により UI は **同一 `holter-http-api` バイナリに埋め込み**されます。追加フロント成果物や別 UI サーバーは不要です。正本 URL は **`/ui/`**（Windows／Linux 同一）。

- [ ] ブラウザで `http://<host>:<port>/ui/` が開き、日本語コンソールが表示される
- [ ] ヘルス確認が画面上で成功／失敗を判別できる
- [ ] （任意）小 ECL で解析→表示→ダウンロード、または未選択時の日本語エラー（詳細: [../console-ui-smoke.md](../console-ui-smoke.md)）
